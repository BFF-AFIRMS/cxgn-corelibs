...
sub new {
    ...
    $self->{sv_identity} = 95;
    $self->{start_value} =
      1;    #the end_value is initiated when the first member is added
    $self->{end_value} = 1;
...
sub remove_gappy_cols {
    ...
    my $oseqs = $self->get_overlap_seqs($mxgps)
      ; # ref to hash with id/seq pairs. These are seqs with only the ungappy columns
    my $cols_left;
...
#	print STDERR  "After winnowing cols with max_gaps = $mxgps.  Cols remaining: ", $cols_left, "\n";
    ...
    foreach my $id ( keys %$oseqs ) {
        my $seq = $oseqs->{$id};
        my $member =
          CXGN::Phylo::Alignment::Member->new( id => $id, seq => $seq );
...
sub set_tmp_dir {
    ...
    if ( -d $dir ) {
        ...
        $self->{tmp_dir} =
            $vhost->get_conf('basepath')
          . $vhost->get_conf('tempfiles_subdir')
          . '/align_viewer';
        unless ( -d $self->{tmp_dir} ) {
            warn "Temporary directory does not exist: "
              . $self->{tmp_dir}
...
sub set_start_value {
    ...
    my $value = shift;
    $value =
      $self->check_start_value($value);    #check if the start value is correct
    $self->{start_value} = $value;         #the ruler will use this start_value

    foreach ( @{ $self->{members} } )
    {    #set the start_value of each included member
...
sub from_file {
    ...
    while ( my $entry = $parser->next() ) {
        my ( $id, $seq, $species ) =
          ( $entry->{id}, $entry->{seq}, $entry->{species} );
        chomp $seq;
        my $len    = length($seq);
        my $member = undef;
        eval {
...
sub add_member {
    ...
    if ( ! @{ $self->{members} } ) {
        ...
        if ( $len == $self->{seq_length} ) {
            ...
            unless ( $seq =~ /-/ ) {
                warn
"Sequence w/ id $id does not have same length, assuming unaligned sequences are provided";
                $self->{unaligned} = 1;
...
#we are assuming that muscle will be run via $self->run_muscle();
                ...
                die
"Error in adding pre-aligned sequence $id, length ($len) not the same as overall alignment ("
                  . $self->{seq_length} . ")\n";
            }
        }
        else {
...
sub get_nongaps_hash
{  # this returns a ref to a hash with ids as keys, numbers of nongaps as values
    my $self = shift;
    my %nongaps;
    foreach ( @{ $self->{members} } ) {
        next if ( $_->is_hidden() );
        $nongaps{ $_->get_id() } = $_->get_nongaps()
          ;    # this get_nongaps is a method of Member, returns # of nongaps
    }
    return \%nongaps;
}

sub get_gaps_nongaps
{ # this returns just the total number of nongap characters in the alignment's non-hidden members.
    my $self    = shift;
    my $nongaps = 0;
    foreach ( @{ $self->{members} } ) {
        next if ( $_->is_hidden() );
        $nongaps += $_->get_nongaps()
          ;    # this get_nongaps is a method of Member, returns # of nongaps
    }

#	print "in get_gaps_nongaps: ", $self->get_seq_length()*$self->get_visible_member_nr() - $nongaps,  "   ",  $nongaps, "\n";
    return (
        $self->get_seq_length() * $self->get_visible_member_nr() - $nongaps,
        $nongaps );
}
...
sub layout {
    ...
    $self->_add_cvg_chart( "Coverage %", $self->get_ngap_pct )
      if ( $option =~ /^(c|s)/i && !$self->{chart} );
...
#into account, but this is much easier for now:
    my $width_adjustment =
      $label_gap + $self->get_left_margin + $self->get_right_margin;
    $width_adjustment = $self->{width_adjustment}
      if exists $self->{width_adjustment};
    ...
    if ( $option eq 'c' || $option eq 'a' || $option eq 's' )
    {    #alignment shown, default layout if member values not set
        my $align_v_offset;
        ( $option eq 'c' || $option eq 's' )
          and $align_v_offset = $ruler_top_margin + $chart_height + 40;
...
#Generate a image object for the ImageObject (member, ruler and chart) to render
    unless ( $self->{image} ) {
        $self->{image} =
          GD::Image->new( $self->get_width(), $self->get_height() )
    ...
    $self->{image}->filledRectangle( 0, 0, $self->{width}, $self->{height},
        $self->{white} );
    ...
    #explicitly.
    $self->{legend}->render( $self->{image}, $self, $m )
      if ( $self->{legend} && ( $option eq 'c' || $option eq 's' ) );
...
sub render_separate_png_files {
    ...
    foreach ( @{ $self->{members} } ) {
        ...
        open( WF, ">$img_path" );
        my $w =
          $_->get_width() + $self->get_label_gap() + $self->get_right_margin();
...
sub render_png_file {
    ...
    open( F, ">$filename" )
      || die
"Can't open $filename for writing!!! Check write permission in dest directory.";
...
sub get_image_map {
    ...
    if ( $self->{ruler} && $self->get_fasta_temp_file ) {
        ...
        $map_content .=
          $self->{ruler}->get_imagemap_string( $self, $self->get_param );
    }
...
sub get_sv_candidates {
    ...
    my ( $indel, $overlap );
    ( $self->{type} eq 'pep' )
      ? ( $indel = '-' x $self->{indel_limit} )
      : ( $indel = '---' x $self->{indel_limit} );
    ( $self->{type} eq 'pep' )
      ? ( $overlap = $self->{sv_overlap} )
      : ( $overlap = $self->{sv_overlap} * 3 );
    ...
    foreach ( my $i = 0 ; $i < @{ $self->{members} } ; $i++ ) {
        foreach ( my $j = $i + 1 ; $j < @{ $self->{members} } ; $j++ ) {
            ( ( $self->{members}[$i]->get_species() ) ne
                  ( $self->{members}[$j]->get_species() ) )
              and next;
            my ( $ol_seq1, $ol_seq2 ) =
              $self->{members}[$i]->get_clean_member( $self->{members}[$j] );

            ( !( ( $ol_seq1 =~ /$indel/ ) || ( $ol_seq2 =~ /$indel/ ) ) )
              and next;

            my ( $self_id, $other_id ) =
              ( $self->{members}[$i]->get_id(),
                $self->{members}[$j]->get_id() );
            my ( $ob, $pi ) =
              $self->{members}[$i]
              ->calculate_similarity( $self->{members}[$j] );
            if ( ( $ob >= $overlap ) && ( $pi >= $self->{sv_identity} ) ) {
                $sv_candidate_ob{$self_id}{$other_id} = $ob;
                $pi = sprintf( "%.2f", $pi )
                  ;   #truncate the number to two digits after the decimal point
...
sub get_allele_candidates {
    ...
    foreach ( my $i = 0 ; $i < @{ $self->{members} } ; $i++ ) {
        foreach ( my $j = $i + 1 ; $j < @{ $self->{members} } ; $j++ ) {
            ...
            ( ( $ol_seq1 =~ /$indel/ ) || ( $ol_seq2 =~ /$indel/ ) )
              and next;    #skip if the aequence pair have long indel
...
sub get_overlap_score {
    ...
    my $score = 0;
    foreach ( my $i = $self->{start_value} - 1 ;
        $i < $self->{end_value} - 1 ; $i++ )
    {
...
sub get_overlap_seqs {
      ...
      )
    {
        ...
        foreach ( @{ $self->{members} } ) {
            ...
            my $id = $_->get_id();
            $overlap_seqs{$id} = ""
              unless ( $overlap_seqs{$id} )
              ; # make sure there is a key/val pair for each id (with val empty string).
...
sub get_overlap_num {
    ...
    { # new way. also works but uses get_overlap_cols, which is more general as can allow >0 gaps in a col.
...
sub get_overlap_cols {
    my $self     = shift;
    my $max_gaps = shift
      ; # this is the max number of gaps allowed in a column which is kept in the overlap
...
sub get_ngap_pct {
        ...
        $i < $self->{end_value} ; $i++ )
    {
        ...
        if ( $pct > 0 ) {
            ...
            if ( $max_ind > 1 ) {
                $conservation_hash{$i} =
                  sprintf( "%.2f", ( $max_ind / $base_count ) * 100 );
            }
            if ( $max_type > 1 && $self->{type} eq 'pep' ) {
                $type_hash{$i} =
                  sprintf( "%.2f", ( $max_type / $base_count ) * 100 );
            }
        }
        else {
...
sub highlight_domains {
  ...
  #Retrieved from storable hash of gene objects, saving resources when possible:
    ...
    foreach my $member ( @{ $self->{members} } ) {
        ...
        my ( $lb, $rb ) =
          $member->translate_positions_to_seq( $start_value, $end_value );
        unless ( !$sigpos || $lb > ( $cleavage + 1 ) ) {
            my @pepcolor = ( 40, 180, 40 );
            $member->add_region( "Signal Peptide",
                1, $cleavage - 1, \@pepcolor )
              if $sigpos;
            $sigpos_found = 1;
        }
        ...
        foreach (@domains) {
            my ( $lb, $rb ) =
              $member->translate_positions_to_seq( $start_value, $end_value );
            unless ( ( defined $rb && $_->{dom_start} > $rb )
                || ( $_->{dom_end} < $lb ) )
            {
                $domains{ $_->{dom_desc} }->{count}++;
                $domains{ $_->{dom_desc} }->{length_tally} +=
                  ( $_->{dom_end} - $_->{dom_start} );
                $domains{ $_->{dom_desc} }->{tooltip} = $_->{dom_full_desc}
                  if $_->{dom_full_desc};
                $domains{ $_->{dom_desc} }->{url} =
                  identifier_url( $_->{dom_interpro_id} );
            }
            else {
                my $debug =
                    "Dom-start: "
    ...
    $self->add_legend_item( "Signal Peptide", [ 40, 180, 40 ] )
      if $sigpos_found;
    ...
    foreach my $domain (
        sort { $domains{$b}->{length_tally} <=> $domains{$a}->{length_tally} }
        keys %domains )
    {
...
sub run_muscle {
    ...
    if ( $run eq "local" ) {
        ...
        unless ( -x $mt ) {
            warn
"Program 'muscle' not available, alignment will not be performed.";
            return;
        }
        my $wd = &Cwd::cwd();
        chomp $wd;
